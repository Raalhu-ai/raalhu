...
The main process supplies the backend URL to the renderer so session authentication, validation, and model requests share a backend. Official releases embed `RAALHU_RELEASE_API_BASE` during the build; `RAALHU_API_BASE` overrides it at launch for self-hosting. Local development defaults to localhost. See [RELEASING.md](RELEASING.md). Deploy the updated Hono backend before publishing the frontend validation-forwarding change or using desktop validation against a hosted backend.
