...
export const AGENT_TOOLS = [
	{
		functionDeclarations: [
						...
						ingredients: {
							...
							items: {
								...
								properties: {
									...
									unit: {
										type: 'STRING',
										description: 'Unit of measurement. Omit for countable items (e.g. 3 eggs)',
			...
			},
			{
				name: 'show_widget',
				description:
					'Render an interactive visualization inline in chat. Use for: charts (Chart.js), diagrams (SVG/mermaid), mockups (HTML+CSS), interactive explainers (HTML+JS), and illustrative art (SVG). ALWAYS call read_skill with name "visualiser" before your first show_widget call in a conversation to load the full design system and utility classes. LANGUAGE: This is a Dhivehi (ދިވެހި) application — ALL text labels, titles, descriptions, tooltips, and annotations in widgets MUST be in Dhivehi (Thaana script). NEVER use English as the primary text. Thaana text is RTL — use direction:rtl on text containers and text-anchor="end" for right-aligned SVG text. CRITICAL DARK THEME RULES: The app uses a dark theme — NEVER use white, light gray, or any light background colors. NEVER output a full HTML document (no <!DOCTYPE>, <html>, <head>, <body> tags) — only output HTML fragments. Use CSS variables provided by the wrapper: var(--bg) #242526 for backgrounds, var(--bg2) #2d2e2f for surfaces, var(--p) #e4e6eb for text, var(--acc) #7d9fe3 for accents, var(--b) #3a3b3c for borders. NEVER hardcode light colors. Auto-detects mode: if code starts with <svg, SVG mode is used; otherwise HTML mode. A global sendPrompt(text) function is available in HTML mode to send messages back to chat.',
				parameters: {
					type: 'OBJECT',
					properties: {
						title: {
