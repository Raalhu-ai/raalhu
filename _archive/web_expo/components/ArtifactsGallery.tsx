...
import { FileText, FileSpreadsheet, BookOpen, Image, Code } from 'lucide-react';
...
export function ArtifactsGallery({ onBack, onSelectCard, onOpenSession }: ArtifactsGalleryProps) {
	...
	function getFileIcon(filename: string, mimeType: string): React.ReactNode {
		...
		if (['docx', 'doc'].includes(ext)) return <FileText size={20} className="text-muted-foreground" />;
		if (['xlsx', 'xls', 'csv'].includes(ext)) return <FileSpreadsheet size={20} className="text-muted-foreground" />;
		if (ext === 'pdf') return <BookOpen size={20} className="text-muted-foreground" />;
		if (mimeType.startsWith('image/')) return <Image size={20} className="text-muted-foreground" />;
		if (['py', 'js', 'ts', 'html', 'css'].includes(ext)) return <Code size={20} className="text-muted-foreground" />;
							...
							<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
								{filteredCards.map(card => (
									...
									>
										...
										<span className="thaana text-[10px] text-primary mt-2">ތައްޔާރުކުރޭ →</span>
									</button>
								))}
							</div>
						</>
									...
									{userArtifacts.map((art, i) => (
										...
										>
											<div className="flex items-center gap-2 mb-2">
												{getFileIcon(art.filename, art.mimeType)}
