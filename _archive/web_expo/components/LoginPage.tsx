...
import { startLogin, exchangeCode } from '@raalhu/shared';
import { Loader2, ArrowLeft, ClipboardPaste, ExternalLink } from 'lucide-react';
...
export function LoginPage({ onBack, onSuccess }: LoginPageProps) {
	...
	if (submitting) {
		return (
			<div className="h-dvh flex flex-col items-center justify-center px-6 overflow-hidden animate-fade-in" style={{ width: '100%' }}>
	...
	return (
		<div className="h-dvh flex flex-col items-center justify-center px-6 overflow-hidden animate-fade-in" style={{ width: '100%' }}>
			...
			<button
				onClick={onBack}
				className="absolute top-6 start-6 inline-flex items-center gap-1.5 text-sm text-muted-foreground/60 hover:text-muted-foreground transition-colors cursor-pointer bg-transparent border-none"
			>
			...
			<div className="max-w-md w-full mx-auto animate-fade-in-up">
				...
				<div className="flex flex-col gap-6">
					...
					<div className="flex items-start gap-4">
						...
						<div className="flex-1">
							...
							<button
								...
								className="inline-flex items-center gap-3 px-5 h-11 bg-card border border-border/60 rounded-lg hover:bg-accent transition-colors cursor-pointer"
							>
					...
					<div>
						...
						<div className="relative" dir="ltr">
							...
							<input
								...
								className="w-full pl-10 pr-4 h-12 bg-background border border-border/60 rounded-xl text-foreground font-mono text-base placeholder:text-muted-foreground/40 placeholder:font-[var(--font-thaana)] placeholder:text-sm focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/30"
							/>
						</div>
					</div>
					...
					<div className="pt-2">
						<button
							...
							className="thaana w-full h-12 bg-primary text-primary-foreground font-semibold text-lg rounded-xl hover:bg-primary/90 transition-colors duration-150 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
						>
							ކުރިއަށް
						</button>
					</div>
				...
				<div className="text-center mt-6">
					<button
						...
						className="thaana inline-flex items-center gap-1.5 text-sm text-muted-foreground/60 hover:text-muted-foreground transition-colors cursor-pointer bg-transparent border-none"
					>
