# Word Document Creation (python-docx) in Pyodide
...
for cell in header_cells:
    # Cell shading — use parse_xml for reliable shading
...
cell_top.merge(cell_bottom)  # Merges col 0, rows 0-2
```

## Images

```python
# CRITICAL: Image must exist in the virtual filesystem first
# Write image bytes via write_file or execute_python before referencing

doc.add_picture('/workspace/image.png', width=Inches(4))

# With specific height (aspect ratio NOT preserved if both set)
doc.add_picture('/workspace/logo.png', width=Inches(2), height=Inches(1))
...
```python
# Method 1: Add a page break run
doc.add_page_break()

# Method 2: Page break before a paragraph
...
## Table of Contents (via XML — requires Word to update fields)

```python
# TOC is inserted as a field code. Word/LibreOffice updates it on open.
...
instrText = OxmlElement('w:instrText')
instrText.set(qn('xml:space'), 'preserve')
instrText.text = ' TOC \\o "1-3" \\h \\z '  # Headings 1-3, hyperlinks, hide page nums in web
...
# IMPORTANT: Headings MUST use built-in heading styles (Heading 1, Heading 2, etc.)
# for the TOC to pick them up
doc.add_heading('Chapter 1', level=1)
doc.add_heading('Section 1.1', level=2)
```

## Sections (Multiple Page Layouts)

```python
from docx.enum.section import WD_ORIENT

# Add a new section (creates a section break)
new_section = doc.add_section()
new_section.orientation = WD_ORIENT.LANDSCAPE
new_section.page_width, new_section.page_height = new_section.page_height, new_section.page_width

# Each section can have its own headers/footers, margins, orientation
...
for run in p.runs:
    set_run_rtl(run)
    run.font.name = 'MV Typewriter'  # Body text (preloaded at /fonts/)
```

**CRITICAL: Thaana font must be set on the run.** Use `MV Typewriter` for body text or `Sangu Suruhee` for headings — both are preloaded at `/fonts/`. After saving, call `embed_thaana_fonts()` to embed fonts in the document.
...
When creating Thaana documents, set the font name on runs:
```python
...
**IMPORTANT:** Setting the font name alone only tells Word which font to *request*. To guarantee correct rendering on any system, **embed the fonts** using `embed_thaana_fonts()` after saving (see below).
...
After saving a document with Thaana text, call `embed_thaana_fonts()` to embed the actual TTF files into the .docx. This ensures correct rendering on any system, even without the fonts installed.

**Always embed fonts when the document contains Thaana text.**
...
def embed_thaana_fonts(docx_path):
    """Embed MV Typewriter and Sangu Suruhee fonts into a .docx file."""
...
```python
# ❌ WRONG — never use tables as dividers/rules
# Tables have minimum height and render as empty boxes

# ✅ CORRECT — use paragraph borders
from docx.oxml.ns import qn, nsdecls
from docx.oxml import parse_xml
...
```python
doc = Document('/workspace/uploaded.docx')
...
- **Set page size explicitly** — python-docx defaults to A4 (Letter is 8.5×11 in)
- **Landscape: swap width and height manually** after setting `WD_ORIENT.LANDSCAPE`
- **Never use `\n` for line breaks** — use separate Paragraph elements, or `run.add_break()` for soft breaks within a paragraph
- **Never use unicode bullets** (`•`, `\u2022`) — use `List Bullet` / `List Number` styles
- **Never use tables as dividers** — use paragraph borders instead
- **Set column widths on every cell** — without explicit widths, columns auto-size unpredictably
- **Use `parse_xml` for cell shading** — `ShadingType` approach is unreliable; XML is safer
- **Embed Thaana fonts** — call `embed_thaana_fonts()` after saving any document with Thaana text
- **Thaana requires manual RTL + font** — set bidi on paragraph, rtl on run, and a Thaana font on run.font.name
- **TOC requires Word/LibreOffice to update** — the field code is inserted but page numbers populate on open
- **Headings must use built-in styles** for TOC to work (`Heading 1`, `Heading 2`, etc.)
